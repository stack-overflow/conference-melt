...
**Share flow.** On boot with `plan` present: `decodePlan` into `sharedPlan`. While `sharedPlan` is set, `ShareBanner` renders in the shell directly under the day tabs (above the filter-chip row), in every view: "Ktoś udostępnił Ci plan: N wydarzeń" with the noun declined by `plural` ("1 wydarzenie", "2 wydarzenia", "5 wydarzeń") (N = `sharedPlan.ids.length`, plus "M nie pasuje do tej wersji harmonogramu" when `unknown > 0`) with three actions. "Wczytaj" unions `sharedPlan.ids` into `favourites`. "Tylko podgląd" sets `previewPlan = sharedPlan.ids` and switches to the plan view. The close icon dismisses. Every action clears `sharedPlan` and removes `plan` from the hash. When `ids` is empty (every id unknown, or an unrecognised version prefix) the banner says the link points to a schedule version that no longer matches and offers only close.
...
- Every counted noun in the copy is declined by `plural(n, one, few, many)` (`domain/plural.ts`): 1 takes the singular, 2–4 the "few" form except in the 12–14 teens (decided by the last two digits), everything else the "many" form — "1 wydarzenie", "2 wydarzenia", "5 wydarzeń"; likewise "1 konflikt" / "2 konflikty" / "5 konfliktów" and the boot toast "Pominięto 1 zapisane wydarzenie, którego nie ma w tej wersji harmonogramu".
...
*Slot mode.* Rows come from `detectSlots` over the layout set. The scroll container is itself the CSS grid: its direct children are the corner cell, one header cell per column, one rail cell per slot row, one cell wrapper per (column, row) that has content, spanning cards, and the now line. The rail cell shows the slot start in the display font and, when the cluster has more than one distinct start, the range `09:30–10:00` underneath in small text. The grid declares `grid-template-rows: auto` for the header row (grid row 1) and `grid-auto-rows: minmax(min-content, auto)` for the slot rows, and the row floor lives on the items instead: the rail cell and every cell wrapper and spanning card carry `min-height: var(--row-min)`, with `--row-min` written on the grid as an inline style, 64 px compact, 88 px comfortable. A fixed track minimum (`minmax(<rowMin>, auto)`) does not work here: a track with a fixed minimum only grows in the grid algorithm's "maximize tracks" step, which needs positive free space, and the scroll container is always shorter than its content, so rows stayed at `rowMin` and clipped their cards. Sizing the track to `min-content` makes a slot row grow to its tallest wrapper while the per-item `min-height` keeps the floor. The slot rail is wider than the timeline rail so the range label fits on one line: `--rail-width` is overridden to 84 px on the slot scroll container, while the timeline keeps the 56 px token. Slot row `r` (0-based) is grid row `r + 2`; column `c` (0-based) is grid column `c + 2`; the rail is grid column 1. Placement is written as inline `grid-column` and `grid-row` styles on wrappers and spanning card items. A cell wrapper stacks its children vertically with an 8 px gap: continuation stubs first, then cards sorted by start then title.
...
Opens on card click. The side comes from the shared `useTier()` hook (7.10), not from a bespoke width listener: `mobile` gives a bottom sheet at 90 % height, every other tier a 420 px side sheet from the right, whose content scrolls while the grid stays visible. Both trap focus, close on Escape, backdrop click and the close button, and restore focus to the control that opened them (a card's primary button, a list row, a chip).
